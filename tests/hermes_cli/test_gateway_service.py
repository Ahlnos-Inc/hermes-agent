...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_restart_self_enable_failure_leaves_gateway_running(
        self, monkeypatch
    ):
        label = gateway_cli.get_launchd_label()
        domain = gateway_cli._launchd_domain()
        ...
        def fake_run(cmd, **kwargs):
            if cmd == ["launchctl", "enable", target]:
    ...
    def test_launchd_stop_fails_closed_when_disable_fails(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda exact_label: gateway_cli.LaunchdLabelProbe((domain,), (), (), ()),
        )

        with pytest.raises(gateway_cli.LaunchdFenceError):
            gateway_cli.launchd_stop()

        assert calls == [["launchctl", "disable", target]]

    def test_launchd_stop_disable_exit_125_is_a_hard_fence_failure(self, monkeypatch):
        """Exit 125 is not proof that no launchd supervisor can respawn."""
        label = gateway_cli.get_launchd_label()
        domain = gateway_cli._launchd_domain()
        target = f"{domain}/{label}"
        calls = []
    ...
    def test_launchd_stop_retries_newly_registered_exact_domain(self, monkeypatch):
        label = gateway_cli.get_launchd_label()
        stale_domain = gateway_cli._launchd_domain()
        actual_domain = "user/501" if stale_domain != "user/501" else "gui/501"
        calls = []

        def fake_run(cmd, **kwargs):
            calls.append(cmd)
            if cmd == ["launchctl", "disable", f"{stale_domain}/{label}"]:
                raise subprocess.CalledProcessError(125, cmd)
            return SimpleNamespace(returncode=0, stdout="", stderr="")
        ...
        monkeypatch.setattr(
            gateway_cli,
            "_probe_launchd_label_domains",
            lambda exact_label: gateway_cli.LaunchdLabelProbe(
    ...
    def test_launchd_restart_from_stopped_state_enables_before_kickstart(
        self, tmp_path, monkeypatch
    ):
        plist_path = tmp_path / "ai.hermes.gateway.plist"
        plist_path.write_text(gateway_cli.generate_launchd_plist(), encoding="utf-8")
    ...
    def test_launchd_start_precondition_failure_does_not_enable(self, monkeypatch):
        """A rejected plist write cannot clear a maintenance fence."""
        calls = []
        monkeypatch.setattr(
            gateway_cli,
            "_refuse_temp_home_service_write",
            lambda content, kind: True,
        )
        monkeypatch.setattr(
            gateway_cli.subprocess,
            "run",
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: Path("/missing/hermes.plist"))
    ...
    def test_launchd_start_enables_before_bootstrap_when_label_is_disabled(
        self, tmp_path, monkeypatch
    ):
        """A successful bootstrap while disabled is not a start; enable comes first."""
        plist_path = tmp_path / "ai.hermes.gateway.plist"
        plist_path.write_text(gateway_cli.generate_launchd_plist(), encoding="utf-8")
        label = gateway_cli.get_launchd_label()
        domain = gateway_cli._launchd_domain()
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: plist_path)
        monkeypatch.setattr(gateway_cli, "refresh_launchd_plist_if_needed", lambda **kwargs: False)

        def fake_run(cmd, check=False, **kwargs):
            calls.append(cmd)
            if cmd == ["launchctl", "enable", target]:
                state["disabled"] = False
            if cmd[:2] in (["launchctl", "bootstrap"], ["launchctl", "kickstart"]):
    ...
    def test_launchd_install_reenable_precedes_bootstrap(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.hermes.gateway.plist"
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: plist_path)
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: "<plist><string>/Users/alice/.hermes</string></plist>",
        )
        monkeypatch.setattr(gateway_cli, "_refuse_temp_home_service_write", lambda *_: False)
        calls = []
        ...
        service_calls = [
            cmd
            for cmd in calls
            if cmd[:2] in (["launchctl", "enable"], ["launchctl", "bootstrap"])
        ]
        assert service_calls == [
            ["launchctl", "enable", f"{gateway_cli._launchd_domain()}/{gateway_cli.get_launchd_label()}"],
            ["launchctl", "bootstrap", gateway_cli._launchd_domain(), str(plist_path)],
        ]

    def test_launchd_stop_all_fences_exact_installed_profile_labels(
        self, tmp_path, monkeypatch
    ):
        ...
        (agents_dir / "ai.hermes.update-checker.plist").write_text(
        ...
        assert all(
            calls.index(["launchctl", "disable", f"{domain}/{label}"])
            < calls.index(["launchctl", "bootout", f"{domain}/{label}"])
            for label in expected_labels
        )
        assert not any(cmd[:2] == ["launchctl", "enable"] for cmd in calls)

    def test_launchd_named_profile_stop_uses_exact_label(self, tmp_path, monkeypatch):
        ...
        target = "gui/501/ai.hermes.gateway-coder"
        assert calls == [
            ["launchctl", "disable", target],
            ["launchctl", "bootout", target],
        ]

    def test_launchd_status_reports_disabled_fence(self, tmp_path, monkeypatch, capsys):
        plist_path = tmp_path / "ai.hermes.gateway.plist"
        plist_path.write_text(gateway_cli.generate_launchd_plist(), encoding="utf-8")
        label = gateway_cli.get_launchd_label()
        domain = "user/501"
        ...
        def fake_run(cmd, **kwargs):
            if cmd[:2] == ["launchctl", "list"]:
                return SimpleNamespace(returncode=0, stdout='"Label" = "' + label + '";\n', stderr="")
            if cmd[:2] == ["launchctl", "print-disabled"]:
                return SimpleNamespace(returncode=0, stdout=f'"{label}" => true\n', stderr="")
            return SimpleNamespace(returncode=0, stdout="", stderr="")
    ...
    def test_gateway_stop_all_uses_launchd_fence_sweep(self, monkeypatch, capsys):
        ...
        identities = [
            gateway_cli.GatewayProcessIdentity(
                101, 1010, "python -m hermes_cli.main gateway run --replace"
            )
        ]
        ...
        monkeypatch.setattr(
            gateway_cli,
            "launchd_stop_all",
            lambda: calls.append("fence-all") or (2, []),
        )
        monkeypatch.setattr(
            gateway_cli,
            "find_gateway_process_identities_strict",
            lambda **kwargs: identities if not calls or calls[-1] == "fence-all" else [],
        )
        monkeypatch.setattr(
            gateway_cli,
            "terminate_gateway_process_identities_strict",
            lambda captured, **kwargs: calls.append({"strict": captured})
            or tuple(captured),
        )

        gateway_cli.gateway_command(SimpleNamespace(gateway_command="stop", all=True))

        assert calls == ["fence-all", {"strict": identities}]
        assert "Stopped 1 residual gateway process(es)" in capsys.readouterr().out
    ...
    def test_gateway_stop_all_aborts_sweep_when_launchd_fence_is_partial(
        ...
        monkeypatch.setattr(
            gateway_cli, "find_gateway_process_identities_strict", lambda **kwargs: []
        )
        monkeypatch.setattr(
            gateway_cli,
            "kill_gateway_processes",
            lambda **kwargs: pytest.fail("partial fencing must abort the global sweep"),
        )
        ...
        assert excinfo.value.code == 1
        assert "ai.hermes.gateway-coder" in capsys.readouterr().out

    def test_gateway_restart_all_uses_per_target_transaction_without_global_sweep(
        self, monkeypatch
    ):
        calls = []
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: False)
        ...
        monkeypatch.setattr(
            gateway_cli, "launchd_restart_all", lambda: calls.append("restart-all")
        )
        monkeypatch.setattr(
            gateway_cli,
            "kill_gateway_processes",
            lambda **kwargs: pytest.fail("restart --all must not sweep globally"),
        )
        ...
        assert calls == ["restart-all"]

    def test_launchd_unloaded_inventory_is_fenced_in_both_domains_without_bootout(
        self, tmp_path, monkeypatch
    ):
        agents_dir = tmp_path / "Library" / "LaunchAgents"
        agents_dir.mkdir(parents=True)
        ...
        assert ["launchctl", "disable", f"gui/501/{label}"] in calls
        assert ["launchctl", "disable", f"user/501/{label}"] in calls
        assert not any(cmd[:2] == ["launchctl", "bootout"] for cmd in calls)

    def test_launchd_unloaded_profiles_restore_in_each_exact_disabled_domain(
        self, tmp_path, monkeypatch
    ):
        agents_dir = tmp_path / "Library" / "LaunchAgents"
        agents_dir.mkdir(parents=True)
        labels = ("ai.hermes.gateway", "ai.hermes.gateway-coder")
        for label in labels:
            _write_launchd_gateway_plist(agents_dir / f"{label}.plist", label)
    ...
    def test_gateway_stop_all_actual_disable_125_aborts_before_pid_sweep(
        ...
        monkeypatch.setattr(
            gateway_cli,
            "kill_gateway_processes",
            lambda **kwargs: pytest.fail("unsafe fence failure must abort before sweep"),
        )
        monkeypatch.setattr(gateway_cli.subprocess, "run", fake_run)
    ...
    def test_gateway_stop_all_reports_final_scan_failure_after_identity_termination(
        self, monkeypatch, capsys
    ):
        ...
        monkeypatch.setattr(gateway_cli, "find_gateway_process_identities_strict", strict_scan)
        monkeypatch.setattr(
            gateway_cli,
            "launchd_stop_all",
        ...
        monkeypatch.setattr(
            gateway_cli,
            "terminate_gateway_process_identities_strict",
            lambda captured, **_kwargs: tuple(captured),
        )
    ...
    def test_gateway_stop_all_strict_termination_permission_failure_names_pid(
        ...
        identity = gateway_cli.GatewayProcessIdentity(
            4242, 42420, "python -m hermes_cli.main gateway run --replace"
        )
        monkeypatch.setattr(
            gateway_cli,
            "find_gateway_process_identities_strict",
    ...
    def test_gateway_stop_all_aborts_when_sweep_safe_is_false_without_failure_text(
        ...
        monkeypatch.setattr(
            gateway_cli, "find_gateway_process_identities_strict", lambda **_kwargs: []
        )
        monkeypatch.setattr(
            gateway_cli,
            "launchd_stop_all",
    ...
    def test_launchd_restore_replaced_plist_aborts_before_bootstrap(
        ...
        monkeypatch.setattr(
            gateway_cli,
            "_launchd_enable",
            lambda *args: enable_calls.append(args),
        )
    ...
    def test_launchd_restore_path_swap_before_fence_clear_has_zero_enable(
        self, tmp_path, monkeypatch
    ):
        plist_path = tmp_path / "ai.hermes.gateway-coder.plist"
        _write_launchd_gateway_plist(plist_path, "ai.hermes.gateway-coder")
        ...
        _write_launchd_gateway_plist(replacement, "ai.hermes.gateway-coder")
        document = plistlib.loads(replacement.read_bytes())
        document["Comment"] = "swapped before restore"
        replacement.write_bytes(plistlib.dumps(document))
        os.replace(replacement, plist_path)
        enables = []
        monkeypatch.setattr(
            gateway_cli,
            "_launchd_enable",
            lambda *args: enables.append(args),
        )
    ...
    def test_launchd_restore_zero_exit_without_live_pid_is_red(
        self, tmp_path, monkeypatch
    ):
        ...
        _write_launchd_gateway_plist(plist, "ai.hermes.gateway")
        target = "gui/501/ai.hermes.gateway"
        # Use the all-profile simulator without starting any real service.
        from tests.hermes_cli.test_gateway_launchd_all import _FakeLaunchd, _job
        ...
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "_launchd_pid_is_live", lambda *_args: False)
    ...
    def test_launchd_release_missing_snapshot_needing_enable_is_fail_closed(
        self, tmp_path, monkeypatch
    ):
        plist_path = tmp_path / "ai.hermes.gateway-coder.plist"
        _write_launchd_gateway_plist(plist_path, "ai.hermes.gateway-coder")
        ...
        monkeypatch.setattr(
            gateway_cli, "_launchd_enable", lambda *args: enables.append(args)
        )
    ...
    def test_gateway_stop_all_reports_new_process_without_signalling_it(
        self, monkeypatch, capsys
    ):
        old = gateway_cli.GatewayProcessIdentity(
            101, 1010, "python -m hermes_cli.main gateway run --replace"
        )
        new = gateway_cli.GatewayProcessIdentity(
            202, 2020, "python -m hermes_cli.main gateway run --replace"
        )
        inventory = iter(([old], [new]))
        captured = []
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_windows", lambda: False)
        ...
        monkeypatch.setattr(
            gateway_cli,
            "terminate_gateway_process_identities_strict",
            lambda items, **_kwargs: captured.extend(items) or tuple(items),
        )
        ...
        assert excinfo.value.code == 1
        assert captured == [old]
        assert "not signalled" in capsys.readouterr().out
    ...
    def test_gateway_stop_all_launchagents_permission_error_fails_before_sweep(
        ...
        monkeypatch.setattr(
            gateway_cli,
            "kill_gateway_processes",
            lambda **kwargs: pytest.fail("inventory failure must abort before sweep"),
        )
        ...
        assert excinfo.value.code == 1
        assert "Could not enumerate installed LaunchAgents" in capsys.readouterr().out
    ...
    @pytest.mark.parametrize(
        ...
        [
            b"not a plist",
            plistlib.dumps(
                {"Label": "ai.hermes.gateway-other", "ProgramArguments": ["/bin/false"]}
            ),
            plistlib.dumps(
                {
                    "Label": "ai.hermes.gateway-coder",
    ...
    def test_gateway_stop_all_matching_foreign_plist_is_untouched(
        self, tmp_path, monkeypatch, original, capsys
    ):
        agents_dir = tmp_path / "Library" / "LaunchAgents"
        agents_dir.mkdir(parents=True)
        path = agents_dir / "ai.hermes.gateway-coder.plist"
        ...
        monkeypatch.setattr(
            gateway_cli,
            "kill_gateway_processes",
            lambda **kwargs: pytest.fail("foreign inventory must abort before sweep"),
        )
    ...
    def test_launchd_start_fails_closed_when_bootstrap_identity_is_unverified(
        self, tmp_path, monkeypatch, capsys
    ):
        plist_path = tmp_path / "ai.hermes.gateway.plist"
        plist_path.write_text(gateway_cli.generate_launchd_plist(), encoding="utf-8")
        ...
        with pytest.raises(gateway_cli.LaunchdAllOperationError):
            gateway_cli.launchd_start()
        assert "background process" not in capsys.readouterr().out.lower()

    def test_launchd_install_fails_closed_on_unverified_bootstrap_5(
        self, tmp_path, monkeypatch, capsys
    ):
        plist_path = tmp_path / "ai.hermes.gateway.plist"
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: plist_path)
...
class TestLaunchctlBootstrapEioRetry:
    """Exit-5 bootstrap recovery preserves exact concurrent registrations."""
    ...
    def test_eio_same_domain_registration_is_converged_without_bootout(
        self, monkeypatch
    ):
        calls = []
        ...
        monkeypatch.setattr(
            ...
            "_probe_launchd_label_domains",
            lambda _label: _label_probe(
                registered=(self.DOMAIN,),
                absent=("user/501",),
                pids=((self.DOMAIN, 4242),),
            ),
        )

        assert gateway_cli._launchctl_bootstrap(
    ...
    def test_persistent_eio_reraises_for_domain_fallback(self, monkeypatch):
        # An exit-5 probe with no exact registration fails closed and never
        # grants permission to bootout.
        calls = []
