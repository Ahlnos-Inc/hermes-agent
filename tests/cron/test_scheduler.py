...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_auth_fallback_switches_provider_and_model_together(self, tmp_path):
        ...
        assert success is True
        assert error is None
        # Fork convention (BUILD-392 runtime routing): the primary resolution
        # passes the configured provider as `requested` (plus route_config);
        # the drift invariant under test — provider and model switch
        # ATOMICALLY to the fallback entry — is asserted below unchanged.
