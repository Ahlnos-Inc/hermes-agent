...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        # An explicit per-process override (constructor arg / --reasoning-effort
        # / HERMES_REASONING_EFFORT) always wins, mirroring the chokepoint's
        # "caller-resolved overrides win first" contract. Otherwise resolve
        # per-model override > global reasoning_effort through the shared
        # chokepoint in hermes_constants (Closes #21256).
        ...
        try:
            ...
            if width < 52:
                ...
                if width < 76:
                    ...
                    # Position 10: idle time since the last final agent response
                    idle_since = snapshot.get("idle_since")
                    if idle_since:
                        frags.append(("class:status-bar-dim", " │ "))
