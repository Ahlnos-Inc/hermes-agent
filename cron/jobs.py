...
def load_jobs() -> List[Dict[str, Any]]:
    ...
    if not jobs_file.exists():
        _warn_missing_jobs_file_once(jobs_file)
        return []
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        ...
        _missing_jobs_file_warned.discard(jobs_file)
    except BaseException:
        try:
            os.unlink(tmp_path)
        except OSError:
...
# fallback_providers / profile validation helpers are Ahlnos-local additions,
# kept in the fork-owned cron/ahlnos_jobs_ext.py (which upstream never touches)
# so upstream merges cannot silently drop them. Re-exported here so existing
# callers — including tools/cronjob_tools.py — keep using `from cron.jobs
# import _normalize_fallback_providers`.
from cron.ahlnos_jobs_ext import (  # noqa: E402
    _normalize_fallback_providers,
    _normalize_profile,
)
