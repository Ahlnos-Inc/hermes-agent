...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    try:
        ...
        # to keep the current URL. See #47828. ``target_base_url`` is the
        # fork's resolved/normalized base_url (v1-stripped, local-endpoint
        # aware) — empty iff the raw base_url was empty.
        ...
        if (new_provider or "").strip().lower() == "moa":
            ...
            # target_request_timeout == get_provider_request_timeout(new_provider,
            # new_model) computed at switch entry; provider/model are already
            # swapped here, so this equals upstream's inline recompute.
