...
def _dispatch_nonstreaming_api_request(
    ...
    *,
    ...
    bedrock_client_holder: dict | None = None,
):
    ...
    The worker path additionally threads its per-attempt reliability machinery:
    ``attempt_budgets`` / ``attempt_started_monotonic`` bound the Bedrock boto3
    socket timeout to the remaining attempt budget (raising
    ``AttemptDeadlineExceeded`` when already elapsed), and ``bedrock_client_holder``
    receives the request-scoped Bedrock client + owner thread so the caller's
    watchdog / ``finally`` can abort and close it. The inline/cron path leaves
    all three ``None`` — no budget threading, cached Bedrock client — because
    cron is already per-job timeout-bounded (see ``direct_api_call``). Local-model
    leasing stays in the worker caller so its release-error accounting keeps
    surfacing through ``result``.
    """
    ...
    if agent.api_mode == "bedrock_converse":
        ...
        # Worker path threads the per-attempt deadline so the boto3 socket
        # timeout never outlives the remaining attempt budget; the inline/cron
        # path passes no budget and gets the cached client (pre-extraction
        # behavior). _remaining_attempt_seconds raises AttemptDeadlineExceeded
        # when the budget is already spent before the transport connects.
        remaining = None
        if attempt_budgets is not None and attempt_started_monotonic is not None:
        ...
        if bedrock_client_holder is not None:
...
def direct_api_call(agent, api_kwargs: dict):
    ...
    try:
        # Cron deliberately dispatches WITHOUT a local-model lease and WITHOUT
        # per-attempt budgets (operator-approved 2026-07-17): cron jobs are
        # already per-job timeout-bounded, and reintroducing leasing here would
        # revive the nested-pool lock contention that #62151 removes. The httpx
        # request_timeout on the inline client bounds a hung provider instead.
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    def _call():
        ...
        try:
            ...
            # detectors can force-close the worker's connection. The attempt
            # budgets + bedrock holder thread this worker's per-attempt deadline
            # and request-scoped Bedrock client back through the shared dispatch.
            result["response"] = _dispatch_nonstreaming_api_request(
                ...
                attempt_budgets=_attempt_budgets,
                attempt_started_monotonic=_attempt_started_monotonic,
                bedrock_client_holder=bedrock_client_holder,
            )
    ...
    # uses a generous bounded deadline. Request size must never disable that
    # deadline entirely: the backend can accept an arbitrarily large request
    # and still wedge before emitting its first event. A second failure mode
    # emits an opening SSE frame and then stalls forever in SSL
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions():
        ...
        if hasattr(stream, "choices"):
            # A completed response object is itself a provider event: mark it so
            # the first-event attempt budget doesn't spuriously fire (#62151 fork
            # machinery threaded through the upstream discriminator).
