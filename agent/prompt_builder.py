...
KANBAN_GUIDANCE = (
    ...
    "every few minutes during long subprocesses. "
    "**If your task may run longer than 1 hour, "
    ...
    "reclaim re-queues the task as `ready` without penalty, but your run's "
    "progress is lost.\n"
    "4. **Block on decisions or side-effect boundaries.** If you need a "
    "human decision you cannot infer (UX choice, paywalled source, peer output "
    "you need first), call `kanban_block(reason=\"...\")` and stop. "
    "A reviewer/verifier finding changes requested must use "
    "`kanban_request_rework(finding=..., fix_task_id=... or fix={...}, "
    "request_key=...)`; `kanban_block(kind=\"needs_input\")` is only for "
    "an actual human decision. "
    "GitHub push/open-PR auth is not a human blocker when a releaser exists: call "
    "`kanban_request_publication`, not `kanban_complete` for unpublished "
    "commit here. Pass exact SHA, workspace path, remote ref, stable request_key; "
    "it creates the publisher parent and parks this card until readback proves "
    "SHA. At an auth boundary with no lane, report any external side-effect failure: "
    "`kanban_comment` evidence, then `kanban_block(kind=\"capability\")` with "
    "a ≤160-char signal; do not burn iterations.\n"
    ...
    "that need review, use `kanban_request_rework`, not a human-input block.\n"
    ...
    "a high-level goal), compile once with `kanban_compile_workflow`: give it a "
    "stable workflow_key and idempotency_key, keyed steps with explicit assignees "
    "and parent step keys, and exactly one terminal finalizer/synthesizer/reporter. "
    "After success, return the workflow and terminal task ids and yield \u2014 "
    "no polling or recreating cards. If the compiler tool is absent, decompose "
    "with `kanban_create`, then finish the planner task. "
    "Do NOT execute the routed work yourself.\n"
    "\n"
    ...
    "and express dependencies via parent step keys in the compiled graph, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    ...
    "- Do not call `clarify` headless; comment the context, then "
    "`kanban_block(reason=...)` so the task surfaces for input.\n"
