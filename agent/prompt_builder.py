...
KANBAN_GUIDANCE = (
    ...
    "boundary no profile can resolve, push/open-PR failure, external "
    "side-effect failure, or final publish block, first "
    "`kanban_comment` structured metadata/artifacts/commit "
    "SHA/verification/failure output, then immediately `kanban_block` with a "
    "concise ≤160-char signal; do not burn iterations.\n"
    ...
    "After success, return the workflow and terminal task ids, then "
    "yield; do not poll or recreate intermediate cards. Use `kanban_create` only "
    "for a later one-off follow-up. If the compiler tool is absent, use "
    "`kanban_create` for task-scoped decomposition, then finish the planner task. "
