...
KANBAN_GUIDANCE = (
    ...
    "Rework: `kanban_request_rework(...)`; pass declared "
    "`human_gate_task_id`. Blockers: full stable "
    "`{rework: {open_blockers: [{key, summary}, ...]}}`, never B67/B68 prose. "
    "`kanban_block(kind=\"needs_input\")` is only for "
    "an actual human decision. "
    "A push/open-PR failure is not a human blocker when a releaser exists: for "
    "an unpublished commit call `kanban_request_publication`, never "
    "`kanban_complete`. Pass SHA, workspace, remote ref, request_key; it parks "
    "this card until readback proves SHA. At an "
    "auth boundary with no lane, or any external side-effect failure, "
    "`kanban_comment` the commit SHA/verification/failure output, then "
    "`kanban_block(kind=\"capability\")` with a ≤160-char signal; do not burn "
    "iterations.\n"
