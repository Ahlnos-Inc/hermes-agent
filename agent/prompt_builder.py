...
KANBAN_GUIDANCE = (
    ...
    "GitHub push/open-PR auth is not a human blocker when an orchestrator or "
    "publisher lane exists: `kanban_complete` implementation with branch, commit "
    "SHA, changed files, and verification so publisher can push. For an auth "
    "boundary no profile can resolve, or any push/open-PR failure or external "
    "side-effect failure: `kanban_comment` the structured evidence (commit "
    "SHA/verification/failure output), then `kanban_block` with a ≤160-char "
    "signal; do not burn iterations.\n"
