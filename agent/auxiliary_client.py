...
# Fork ``runtime`` field (native runtime aux routing) kept alongside upstream's
# context-local rework. Field order below MUST match ``_MAIN_RUNTIME_FIELDS``
# (runtime, provider, model, base_url, api_key, api_mode, auth_mode).
...
def set_runtime_main(
    ...
    *,
    ...
    global _RUNTIME_MAIN_RUNTIME, _RUNTIME_MAIN_AUTH_MODE, _RUNTIME_MAIN_COMPAT_SNAPSHOT
    # ``runtime`` is a str param (fork native-runtime identity); the context dict
    # is named ``runtime_dict`` to avoid shadowing it.
    runtime_dict = {
    ...
    token = _RUNTIME_MAIN_CONTEXT.set(runtime_dict)
    with _RUNTIME_MAIN_COMPAT_LOCK:
        (
            ...
            _RUNTIME_MAIN_API_MODE,
            _RUNTIME_MAIN_AUTH_MODE,
        ) = (runtime_dict[field] for field in _MAIN_RUNTIME_FIELDS)
