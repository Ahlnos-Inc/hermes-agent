...
class AIAgent:
    ...
    def __init__(
        ...
        init_agent(
            ...
            total_attempt_timeout_seconds=total_attempt_timeout_seconds,
            first_event_timeout_seconds=first_event_timeout_seconds,
            local_model_max_wait_seconds=local_model_max_wait_seconds,
            local_model_lease_ttl_seconds=local_model_lease_ttl_seconds,
    ...
    def _persist_session(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        def _do_persist():
            ...
            # Kanban delivery-policy withholding: during an authority
            # outage/revocation, project withheld tool/assistant tail onto a
            # fixed receipt for the JSON snapshot only (never the live dict or
            # the durable DB rows, which the finalizer owns).
            ...
            if _delivery_policy is not None:
                try:
                    ...
                    # Policy failures are fail-closed at construction. If even
                    # the receipt projection fails, skip the optional JSON
                    # snapshot; the DB flush below applies its own containment.
                    _log_messages = []
            self._save_session_log(_log_messages)
            _result = self._flush_messages_to_session_db(messages, conversation_history)
        ...
        try:
            ...
            for _msg_idx, msg in enumerate(messages):
                ...
                if is_current_turn_user and msg.get("role") == "user":
                    # Fork: internal_control turns persist the user row as
                    # role="system" via _persist_user_role_override.
    ...
    def _run_codex_app_server_turn(
        ...
        *,
        ...
        return run_codex_app_server_turn(self, user_message=user_message, original_user_message=original_user_message, messages=messages, effective_task_id=effective_task_id)
